    ...
    def main():
        ...
        if mixer.music.get_busy():
            if (length*1000-get_pos()) < 200:  # 播完了换下一首
                ...
                lyric_window_pause_Button.configure(image=play_photo)
                play_end()
                playnext()
        if if_lyric:
            lyric_num = 0
    ...
    try:
        ...
        try:
            ...
            if autocolor_str.get() == 'True':
                ...
                lyric_window_title.configure(background=color)
        ...
        for i in [pause_Button,next_Button,last_Button,lyric_window_last_Button,lyric_window_pause_Button,lyric_window_next_Button]:
            i['state'] = 'normal'
...
def play():
    ...
    lyric_window_pause_Button.configure(image=pause_photo)
    set_pos(0)
...
# 桌面歌词
lyric_window = Toplevel(cursor='fleur')
...
lyric_window_next_Button.pack(side='left',padx=2)
lyric_window_close.pack(side='right',padx=4)
lyric_window_title.pack(side='top',fill='x',expand=True)
...
lyric_Label.bind('<Double-Button-1>',lambda a: enabled_desktop_lyric())
...
lyric_window.bind('<Control-Right>',lambda a: playnext())
lyric_window.bind('<Control-Left>',lambda a: playlast())
