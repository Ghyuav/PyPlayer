...
def load_file(path=''):
    ...
    try: # 若data不存在则不支持播放，直接跳过
        if data:  # 不知什么原因可能返回为None
            pass
        else:
            data = ['', '', '', '', '']
...
ask_file_Button = Button(buttons_Frame, text="打开文件",
                         bootstyle=('info', 'outline'), command=open_file)
pause_Button = Button(buttons_Frame, text='⏯️',
                      bootstyle=('info', 'outline'), command=pause)
next_Button = Button(buttons_Frame, text='⏭️',
                      bootstyle=('info', 'outline'), command=playnext)
last_Button = Button(buttons_Frame, text='⏮️',
                      bootstyle=('info', 'outline'), command=playlast)
...
last_Button.pack(side='left', fill='x', padx=10, pady=5)
pause_Button.pack(side='left', fill='x', padx=5, pady=5)
next_Button.pack(side='left', fill='x', padx=10, pady=5)
